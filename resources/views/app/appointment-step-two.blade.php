        ...
        <div class="relative max-w-screen-xl z-20 container flex flex-col items-center justify-center gap-4 mx-auto py-24">
            ...
            <form class="mt-8 flex flex-col items-center gap-8" action="{{ route('appointment.step-two-submit') }}" method="POST">
                ...
                <div class="flex flex-col gap-4 mt-8">
                    ...
                    <div class="w-80 flex">
                        <input {{ str_contains(session('care'), "Prise") ? "checked" : "" }} type="checkbox" name="prise" id="prise" class="peer hidden" value="Prise de sang">
                        <label for="prise" class="w-80 text-center btn-outline peer-checked:bg-red-damask-400 peer-checked:text-white">
                            prise de sang
                        </label>
                    </div>
                    <div class="w-80 flex">
                        <input {{ str_contains(session('care'), "Injection") ? "checked" : "" }} type="checkbox" name="injection" id="injection" class="peer hidden" value="Injection">
                        <label for="injection" class="w-80 text-center btn-outline peer-checked:bg-red-damask-400 peer-checked:text-white">
                            injection
                        </label>
                    </div>
                    <div class="w-80 flex">
                        <input {{ str_contains(session('care'), "Aide") ? "checked" : "" }} type="checkbox" name="toilette" id="toilette" class="peer hidden" value="Aide à la toilette">
                        <label for="toilette" class="w-80 text-center btn-outline peer-checked:bg-red-damask-400 peer-checked:text-white">
                            aide à la toilette
                        </label>
                    </div>
                    <div class="w-80 flex">
                        <input {{ str_contains(session('care'), "Perfusion") ? "checked" : "" }} type="checkbox" name="perfusion" id="perfusion" class="peer hidden" value="Perfusion">
                        <label for="perfusion" class="w-80 text-center btn-outline peer-checked:bg-red-damask-400 peer-checked:text-white">
                            perfusion
                        </label>
                    </div>
                    <div class="w-80 flex">
                        <input {{ str_contains(session('care'), "Glycémie") ? "checked" : "" }} type="checkbox" name="glycemie" id="glycemie" class="peer hidden" value="Glycémie">
                        <label for="glycemie" class="w-80 text-center btn-outline peer-checked:bg-red-damask-400 peer-checked:text-white">
                            glycémie
                        </label>
                    </div>
                    <div class="w-80 flex">
                        <input {{ str_contains(session('care'), "Séance") ? "checked" : "" }} type="checkbox" name="osteo" id="osteo" class="peer hidden" value="Séance d'ostéopathie">
                    ...
                    <div class="w-80 flex border-b-2 border-black mx-auto mt-8">
                        <input value="{{ session('autre') ?? "" }}" type="text" name="autre" id="autre" class="border-none w-80 neulis placeholder:text-black placeholder:font-bold" placeholder="Autre">
                    </div>
                </div>
                <button type="submit" class="w-80 btn mt-8">poursuivre</button>
                <a href="{{ route('appointment.step-one') }}" class="w-80 btn-outline text-center cursor-pointer">retour</a>
            </form>
        </div>
    </section>
@endsection
